...
<cfif isDefined("form.issueID")>
	<cfset application.issue.update(form.issueID,form.projectid,form.ticketPrefix,form.issue,form.detail,form.type,form.severity,form.status,form.assignedTo,form.relevantURL,session.user.userid)>
	<cfset application.activity.add(createUUID(),form.projectID,session.user.userid,'Issue',form.issueID,form.issue,'edited')>
	<cflocation url="issues.cfm?p=#form.projectID#" addtoken="false">
