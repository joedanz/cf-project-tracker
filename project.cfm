...
<cfif session.user.admin or project.todolist_view eq 1>
	<cfset todos_overdue = application.todo.get('','','','t.due','',url.p,'',1,'overdue')>
	<cfset todos_upcoming = application.todo.get('','','','t.due','',url.p,'',1,'upcoming','1')>
</cfif>
	...
	<div class="left">
		<div class="main">
			...
			<div class="content">
				...
				<cfif project.mstone_view or project.issue_view or project.todolist_view>
					...
					<cfif project.todolist_view>
						<cfquery name="todos_next_14" dbtype="query">
							select * from todos_upcoming where due <= 
							<cfqueryparam value="#CreateODBCDate(DateAdd("d",13,Now()))#" cfsqltype="CF_SQL_DATE" />
						</cfquery>
					</cfif>
					
					<cfif (project.mstone_view and ms_next_14.recordCount) or (project.issue_view and issues_next_14.recordCount) or (project.todolist_view and todos_next_14.recordCount)>
						...
						<table border="0" cellpadding="0" cellspacing="1" width="100%" id="milestone_cal">
							...
							<cfloop index="i" from="0" to="13">
								<cfif i mod 7 eq 0><tr></cfif>
									...
									<cfif project.todolist_view>
										<cfquery name="todays_todos" dbtype="query">
											select todoid,task from todos_upcoming where due = 
											<cfqueryparam value="#CreateODBCDate(DateAdd("d",i,Now()))#" cfsqltype="CF_SQL_DATE" />
										</cfquery>
									</cfif>
									<cfif i eq 0>
									...
									<cfif project.todolist_view and todays_todos.recordCount>
										<cfloop query="todays_todos">
											<li><a href="todo.cfm?p=#url.p#&amp;t=#todoID#">#task#</a> (todo)</li>
										</cfloop>
									</cfif>
									</ul>
								</td>
					...
					<cfif project.todolist_view and todos_upcoming.recordCount>
						<div class="mb5 b" style="border-bottom:1px solid ##000;">
							<span style="float:right;font-size:.75em;"><a href="##" onclick="upcoming_todos('#url.p#','1');$(this).addClass('subactive');$('##t_threem').removeClass('subactive');$('##t_all').removeClass('subactive');return false;" class="sublink subactive" id="t_onem">1 month</a> | <a href="##" onclick="upcoming_todos('#url.p#','3');$('##t_onem').removeClass('subactive');$(this).addClass('subactive');$('##all').removeClass('subactive');return false;" class="sublink" id="t_threem">3 months</a> | <a href="##" onclick="upcoming_todos('#url.p#','');$('##onem').removeClass('subactive');$('##t_threem').removeClass('subactive');$(this).addClass('subactive');return false;" class="sublink" id="t_all">All</a></span>
							Upcoming Todos</div>	
					...
					<table class="activity table table-striped" id="activity">
						...
						<cfloop query="activity">						
							...
							<td><cfswitch expression="#type#">
									...
									<cfcase value="File"><a href="file.cfm?p=#url.p#&amp;f=#id#">#name#</a></cfcase>
